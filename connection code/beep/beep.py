...
class Beep():
   ...
   def loop(self):
        ...
        lcd.message("Hi, please check\nyour schedule***") # or user may consider changing the BCM mode for Beep, like setting a matrix for modifying the BCM mode numbering
        sleep(2)
...
if __name__ == '__main__':     # the program starts now
        ...
        #lcd = LCD()
        beep = Beep()
        beep.setup() # use since the mode for beep can be set as BOARD, not BCM set for LCD class 
        try:
                beep.loop()
